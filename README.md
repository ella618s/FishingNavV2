...
* **資料持久化架構**：整合 SharedPreferences 與 Gson 序列化，實現自訂漁標的離線儲存，確保應用重啟後地理資訊不流失。
...
* **狀態感應式點擊邏輯**： 
    1.單擊標記：即時切換導航目標點，並動態繪製 Polyline 導航路徑。
    2.二次點擊標記：透過 isInfoWindowShown 狀態偵測，實現不依賴長按事件的「編輯對話框」觸發，解決原生手勢衝突問題。
* **資料同步機制**：標記名稱修改後，自動遍歷 mapReference?.overlays 進行即時標籤刷新，並同步更新至手機快取。
* **全域清除邏輯**：實作「一鍵清除導航」與「永久刪除標記」兩階段功能，包含自動關閉 InfoWindow 以維持地圖介面清爽。

## 📱 互動式 GIS 成果展示 (實機驗證)
透過對 Open Data `properties` 欄位之解析，本專案實現了地理區域的中繼資料即時查詢：
* **動態標籤渲染**：針對特定生態保護區或風場區域，實作自動標籤化技術，提升航行者的環境感知能力。
* **點擊交互機制**：整合 OSMdroid `Polygon.setOnClickListener`，點擊圖層即可透過 `Toast` 或 `InfoWindow` 顯示該區域之詳細計畫名稱 (如：`wpname`)。
...
- [x] 自訂漁標點位持久化儲存 (JSON 序列化)
- [x] 動態導航線路切換與路徑點繪製
- [x] 智能點擊交互 (單擊換線、雙擊編輯名稱)
